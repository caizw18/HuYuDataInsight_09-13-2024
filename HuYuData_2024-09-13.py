...
# Fetch Meta's current stock price
ticker = 'META'
data = yf.Ticker(ticker)
current_price = data.history(period='1d')['Close'].iloc[-1]

# Parameters for the Black-Scholes model
S = current_price  # Current stock price of Meta (META)
K = 350  # Strike price (you can change this to any value)
T = (datetime(2024, 12, 31) - datetime.now()).days / 365  # Time to expiration (1 year from today)
r = 0.05  # Risk-free interest rate (e.g., 5%)
sigma = 0.35  # Volatility of the stock price (e.g., 35%)
...
# Print the results
print(f"Meta (META) Current Stock Price: {S:.2f}")
...
# Meta (META) Current Stock Price: 315.30
# Strike Price: 350
# Time to Expiration (years): 1.10
# Risk-Free Interest Rate: 0.05
# Volatility: 0.35
# Call Option Price: 42.85
# Put Option Price: 65.40